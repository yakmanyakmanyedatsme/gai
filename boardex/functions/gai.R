gai_func <- function(rd = F, dta = F, dt = NA, nm = "gai_factors.rds"){
  ...
  gai_ceo_subsample <- function(dftemp){
    ...
    dftemp <- dftemp %>% mutate(ceo_dummy = ifelse(year >= minceoyear & !is.na(minceoyear), 1, 0))  %>% select(directorid, companyid, year, minceoyear, ceo_dummy)
  ...
  tt1 <- gai_ceo_subsample(dt) %>% arrange(directorid, year) %>% select(directorid, companyid, year, minceoyear, ceo_dummy)
  print("finish tt1")
  tt2 <- gai_components(dt) %>% arrange(directorid, year)
  tt2 <- ga_conglomerate_sectors(dt) %>% arrange(directorid, year) 
  print("finish tt2")
  tt <- merge(tt1,tt2,by= c("directorid", "year"))
  tt <- as.data.table(tt)
  write_dta(tt, nm)
  saveRDS(tt, file = nm)
  return(tt)
